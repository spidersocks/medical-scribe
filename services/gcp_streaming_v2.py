...
def _build_v2_recognition_config(language_codes: List[str]) -> speech.RecognitionConfig:
    """Builds the main RecognitionConfig for v2, using the 'chirp' model."""
    ...
    return speech.RecognitionConfig(
        ...
        model="chirp",  # Use the 'chirp' model for broad language support in global region
        features=features
    )
...
def _normalize_v2_langs(codes: List[str]) -> List[str]:
    """Normalizes language codes for v2, mapping zh-TW to cmn-Hant-TW."""
    normalized_codes = []
    for c in codes:
        cl = c.lower()
    ...
    seen = set(); return [c for c in normalized_codes if not (c in seen or seen.add(c))]
...
def register_gcp_streaming_v2_routes(app: FastAPI, *, gcp_project_id: Optional[str] = None, gcp_location: str = "global") -> None:
    ...
    async def client_transcribe_gcp_v2(ws: WebSocket):
        ...
        language_codes = _normalize_v2_langs(req_codes)
        project = _resolve_gcp_project_id(gcp_project_id)

        if not project:
            logger.error("GCP v2: Could not resolve project id."); await ws.close(); return

        # Always use the global endpoint. The 'chirp' model supports all languages globally.
        location = "global"
        logger.info("GCP v2 WS connected. languages=%s | location=%s | project=%s | model=chirp", language_codes, location, project)
        
        speech_client = speech.SpeechClient()
        
        recognition_config = _build_v2_recognition_config(language_codes)
